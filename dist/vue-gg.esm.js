import { scaleLinear, scaleLog, scalePow, scaleTime, scaleDiverging, scaleSequential, scaleOrdinal } from 'd3-scale';
import { interpolateBrBG, interpolatePRGn, interpolatePiYG, interpolatePuOr, interpolateRdBu, interpolateRdGy, interpolateRdYlBu, interpolateRdYlGn, interpolateSpectral, interpolateBlues, interpolateGreens, interpolateGreys, interpolateOranges, interpolatePurples, interpolateReds, interpolateViridis, interpolateInferno, interpolateMagma, interpolatePlasma, interpolateWarm, interpolateCool, interpolateCubehelixDefault, interpolateRainbow, interpolateSinebow } from 'd3-scale-chromatic';
...
function log(domain, range) {
  ...
  var scale = scaleLog().domain(domainCopy).range(range);
  return offsetZeroes(scale);
}

function square(domain, range) {
  var scale = scalePow().exponent(2).domain(domain).range(range);
  return scale;
}

function squareRoot(domain, range) {
  var scale = scalePow().exponent(1 / 2).domain(domain).range(range);
  return scale;
}
...
function temporal$1(domain, range) {
  return scaleTime().domain(domain).nice().range(range);
}
...
function brownBlue(domain) {
  return scaleDiverging(interpolateBrBG).domain(domain);
}

function purpleGreen(domain) {
  return scaleDiverging(interpolatePRGn).domain(domain);
}

function pinkGreen(domain) {
  return scaleDiverging(interpolatePiYG).domain(domain);
}

function purpleOrange(domain) {
  return scaleDiverging(interpolatePuOr).domain(domain);
}
...
function redGray(domain) {
  return scaleDiverging(interpolateRdGy).domain(domain);
}

function redYellowBlue(domain) {
  return scaleDiverging(interpolateRdYlBu).domain(domain);
}

function redYellowGreen(domain) {
  return scaleDiverging(interpolateRdYlGn).domain(domain);
}

function spectral(domain) {
  return scaleDiverging(interpolateSpectral).domain(domain);
} // Sequential
...
function greens(domain) {
  return scaleSequential(interpolateGreens).domain(domain);
}

function greys(domain) {
  return scaleSequential(interpolateGreys).domain(domain);
}

function oranges(domain) {
  return scaleSequential(interpolateOranges).domain(domain);
}

function purples(domain) {
  return scaleSequential(interpolatePurples).domain(domain);
}

function reds(domain) {
  return scaleSequential(interpolateReds).domain(domain);
...
function inferno(domain) {
  return scaleSequential(interpolateInferno).domain(domain);
}

function magma(domain) {
  return scaleSequential(interpolateMagma).domain(domain);
}
...
function warm(domain) {
  return scaleSequential(interpolateWarm).domain(domain);
}

function cool(domain) {
  return scaleSequential(interpolateCool).domain(domain);
}

function cubehelixDefault(domain) {
  return scaleSequential(interpolateCubehelixDefault).domain(domain);
} // Cyclical


function rainbow(domain) {
  return scaleSequential(interpolateRainbow).domain(domain);
}

function sinebow(domain) {
  return scaleSequential(interpolateSinebow).domain(domain);
}
...
var numeric$2 = {
  linear: function linear(domain) {
    return scaleLinear().domain(domain).range([0, 1]);
  }
};
    ...
    tickData: function tickData() {
      if (this.tickValues) {
        ...
        if (this._domainType === 'quantitative') {
          ticks$$1 = ticks.apply(void 0, _toConsumableArray(this._domain).concat([this.tickCount])).map(function (value) {
            return {
              value: value,
              label: format(value)
            };
        ...
        if (this._domainType === 'categorical') {
          ticks$$1 = this._domain.map(function (value) {
            return {
              value: value,
              label: format(value)
            };
        ...
        if (this._domainType === 'temporal') {
          ...
          var scale = scaleTime().domain(this._domain);
          ticks$$1 = scale.ticks(this.tickCount).map(function (value) {
  ...
  computed: {
    ...
    cells: function cells() {
      if (this.gridLines.constructor === Array) {
        ...
        if (this._domainType === 'quantitative') {
          cells = ticks.apply(void 0, _toConsumableArray(this._domain).concat([this.gridLines])).map(function (value) {
            return {
              value: value
            };
          });
...
function interpolatePath(corners, transformer) {
  ...
  for (var i = 0; i < corners.length - 1; ++i) {
    var from = corners[i];
    var to = corners[i + 1];
    var interpolator = interpolate(from, to);
...
function interpolatePathFromFunc(func, transformer, domains) {
  ...
  var points = [];
  var interpolator = interpolate.apply(void 0, _toConsumableArray(domains.x));
