    ...
    >
      ...
      <g v-if="myData">
        ...
        <vgg-section
          ...
          :transform="[
            { scale: {
              scaled0: { prop: 'y', column: 'Price', scale: { domain: 'Price', reverse: true, range: [0, 10] } },
              scaled1: { prop: 'y', column: 'Weight', scale: { domain: 'Weight', range: [0, 10] } },
              scaled2: { prop: 'y', column: 'Horsepower', scale: { domain: 'Horsepower', range: [0, 10] } },
              scaled3: { prop: 'y', column: 'Top Speed', scale: { domain: 'Top Speed', range: [0, 10] } },
              scaled4: { prop: 'y', column: 'Miles per Gallon', scale: { domain: 'Miles per Gallon', reverse: true, range: [0, 10] } },
              scaled5: { prop: 'y', column: 'Zero to 60', scale: { domain: 'Zero to 60', reverse: true, range: [0, 10] } },
              scaled6: { prop: 'y', column: 'Braking 60 mph', scale: { domain: 'Braking 60 mph', reverse: true, range: [0, 10] } },
              scaled7: { prop: 'y', column: 'Torque', scale: { domain: 'Torque', reverse: true, range: [0, 10] } },
              scaled8: { prop: 'y', column: 'Quartermile Second', scale: { domain: 'Quartermile Second', reverse: true, range: [0, 10] } },
              scaled9: { prop: 'y', column: 'Power to Weight', scale: { domain: 'Power to Weight', reverse: true, range: [0, 10] } }
            }
            },
            { transform: df => { log(df); return df; }}
          ]"
        >
          <vgg-map v-slot="{ row, i }">
            ...
            <!-- <vgg-multi-line
              :x="dimensions"
              :y="[row.scaledPrice, row.scaledWW, row.scaledHP]"
              :stroke="row.Color"
            ...
            <!-- <vgg-multi-line
              :x="dimensions"
              :y="[row.scaledPrice, row.scaledWW, row.scaledHP, row.scaledSpeed, row.scaledMiles]"
              :stroke="row.Color"
              @hover="handleHover($event, row, i)"
              @click="handleClick($event, row, i)"
            /> -->
            ...
            <vgg-multi-line
              :x="dimensions"
              :y="[row.scaled0, row.scaled1, row.scaled2, row.scaled3, row.scaled4, row.scaled5, row.scaled6 , row.scaled7 , row.scaled8, row.scaled9]"
              :stroke="row.Color"
              @hover="handleHover($event, row, i)"
              @click="handleClick($event, row, i)"
            />
          ...
          <vgg-y-axis
            ...
            :y1="0"
            :y2="10"
            :scale="flipScale.includes(v) ? {domain: v, reverse: true} : v"
            :title-vjust="1.05"
            :title-hjust="0.5"
            :tick-opacity="3"
            :title-font-size="14"
          ...
          <!-- <vgg-multi-line
            v-if="hoverRow"
            :x="dimensions"
            :y="[hoverRow.scaledPrice, hoverRow.scaledWW, hoverRow.scaledHP]"
            :stroke-width="5"
            stroke="black"
          />
          ...
          <vgg-multi-line
            v-if="clickRow"
            :x="dimensions"
            :y="[clickRow.scaledPrice, clickRow.scaledWW, clickRow.scaledHP]"
            :stroke-width="5"
            stroke="black"
          /> -->
          <!--
...
export default {
  ...
  props: {
    ...
    flipScale: {
      type: Array,
      default: () => ['Miles per Gallon', 'Zero to 60', 'Braking 60 mph']
    },
